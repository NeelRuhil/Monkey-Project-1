// Creating the global variables.

var monkey, monkey_running, monkeyCollide, ground, invisiGround, groundImg, banana, bananaImage, obstacle, obstacleImage;
var FoodGroup, obstacleGroup;
var score = 0,
  bananaScore = 0;
var PLAY = 0;
var END = 1;
var gameState = PLAY;

//________________________________________________________________

// Loading The Animations and images.

function preload() {
  monkey_running = loadAnimation("sprite_0.png", "sprite_1.png", "sprite_2.png", "sprite_3.png", "sprite_4.png", "sprite_5.png", "sprite_6.png", "sprite_7.png", "sprite_8.png");
  
  monkeyCollide = loadAnimation("sprite_1.png");
  groundImg = loadAnimation("ground.jpg")
  bananaImage = loadImage("banana.png");
  obstacleImage = loadImage("obstacle.png");
  
}

//________________________________________________________________

function setup() {
  
  //Making the canvas.
  createCanvas(400, 400);

  // Making the Food and Obstacles Group
  obstacleGroup = createGroup();
  bananaGroup = createGroup();
  
  //creating monkey
  monkey = createSprite(80, 230, 10, 10);
  monkey.scale = 0.12;
  monkey.addAnimation("monkey", monkey_running);
  monkey.addAnimation("collide", monkeyCollide);

  //creating Ground
  ground = createSprite(300, 340, 600, 10);
  ground.scale = 0.7;
  ground.addAnimation("ground", groundImg);
  

  // Making a invisible ground.
  invisiGround = createSprite(300, 278, 600, 7);
  invisiGround.visible = false;
}

//________________________________________________________________

function draw() {
  
  //Making the background to lightblue.   
  background("lightblue");
  
  //making the text black   
  fill("black");
  
  //giving us text banana collected.
  text("Banana Collected : " + bananaScore, 60, 20);
  
  
//________________________________________________________________
  
  // The game state play.   
  if (gameState === PLAY) {
    obstacles();
    bananas();
    score = score + Math.round(getFrameRate() / 60);

    ground.velocityX = -(4 + score * 1.5 / 100);

    if (keyDown("space") && monkey.y >= 235) {
      monkey.velocityY = -13;
    }

    monkey.velocityY = monkey.velocityY + 0.8

    if (ground.x < 0) {
      ground.x = ground.width / 2;
      
    }

    if (monkey.isTouching(bananaGroup)) {
      bananaScore++;
      bananaGroup.destroyEach();

    }
    if (monkey.isTouching(obstacleGroup)) {
      gameState = END;
    }
  }
  
//________________________________________________________________

  //The game state end.   
  if (gameState === END) {
    ground.velocityX = 0;

    monkey.y = 235;
    monkey.scale = 0.12;
    monkey.changeAnimation("collide", monkeyCollide);

    obstacleGroup.setVelocityXEach(0);
    bananaGroup.setVelocityXEach(0);
    obstacleGroup.setLifetimeEach(-1);
    bananaGroup.setLifetimeEach(-1);

    fill("red")
    stroke("black")
    textSize(30);
    text("GAMEOVER", 30, 170);
    fill("black");
    textSize(13);
    text("Press 'R' to play again...", 100, 200);

    if (keyDown("r")) {
      bananaGroup.destroyEach();
      obstacleGroup.destroyEach();
      monkey.changeAnimation("monkey", monkey_running);
      score = 0;
      bananaScore = 0;
      gameState = PLAY;
    }
  }
  
//________________________________________________________________

  // Making the sprites.   
  drawSprites();

  // Making sure that the monkey colides with ground.
  monkey.collide(invisiGround);
}

//________________________________________________________________

// The function banana.
function bananas() {
  if (frameCount % 80 === 0) {

    banana = createSprite(620, 120, 50, 50)
    banana.addAnimation("banana", bananaImage);
    banana.scale = 0.1;
    banana.velocityX = -(4 + score * 1.5 / 100);
    banana.lifetime = 220;
    bananaGroup.add(banana);
    bananaGroup.add(banana);
  }
}

//________________________________________________________________

// The function obstacles.

function obstacles() {
  if (frameCount % 200 === 0) {

    obstacle = createSprite(620, 253, 50, 50);
    obstacle.addAnimation("rock", obstacleImage);
    obstacle.setCollider("circle", 0, 0, 180);
    obstacle.scale = 0.13;
    obstacle.velocityX = -(4 + score * 1.5 / 100);
    obstacle.lifetime = 220;
    obstacleGroup.add(obstacle);

  }
}

//________________________________________________________________